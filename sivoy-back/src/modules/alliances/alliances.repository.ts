...
export class AlliancesRepository {
  constructor(
    @InjectRepository(Alliance) private readonly allianceRepository: Repository<Alliance>,
  ) {}
    ...
    try {
      ...
      throw new BadRequestException(`Error creating alliance: ${error.message}`);
    }
    }

    async updateAlliance(id: string, alliance: UpdateAllianceDto) {
    const updateAlliance = await this.allianceRepository.findOneBy({ id });
    if (!updateAlliance) throw new NotFoundException(`alliance whit ${id} not found`);
